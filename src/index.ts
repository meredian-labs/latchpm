...
import { installAliases, planAfterAudit, removeAliases, type LatchpmAction } from "./flow.js";
import {
  buildNpmCiCommand,
...
import { createProjectWorkflowReport, formatProjectWorkflowReport, type ProjectDependencyAudit, type ProjectWorkflowReport } from "./projectWorkflow.js";
import { createLatchpmInstallReport, createLatchpmReport, formatLatchpmReport, type LatchpmAuditAction, type LatchpmReport } from "./report.js";
...
function addInstallCommand(): void {
  const install = program
    ...
    .action(async (packageSpec: string | undefined, ...rest: Array<Command | GlobalOptions>) => {
...
function addRemoveCommand(): void {
  const remove = program
    ...
    .action(async (packageName: string, ...rest: Array<Command | GlobalOptions>) => {
...
function addRunCommand(): void {
  program
    ...
    .action(async (scriptName: string, scriptArgs: string[] = [], ...rest: Array<Command | GlobalOptions>) => {
      const options = mergeOptions(actionOptions(...rest));
...
async function handlePackage(action: LatchpmAuditAction, packageSpec: string, options: GlobalOptions): Promise<void> {
...
async function handleProjectCommand(
  action: Extract<LatchpmAction, "install" | "ci">,
  options: GlobalOptions,
  command: NpmCommand,
  dependencyOptions: { preferLockfile: boolean }
): Promise<void> {
  const dependencies = await readDirectDependencies(process.cwd(), dependencyOptions);
  const policy = await loadPolicy(options.policy);
  const audits: ProjectDependencyAudit[] = [];
...
async function handleUnauditedCommand(command: NpmCommand, options: GlobalOptions, labels: { title: string; warning: string }): Promise<void> {
  process.stderr.write(`latchpm: ${labels.warning}\n`);
  await maybeRunUnauditedCommand(command, options, labels.title);
}

async function maybeRunUnauditedCommand(command: NpmCommand, options: GlobalOptions, title: string): Promise<void> {
  if (!options.yes) {
...
function emitReport(report: LatchAuditReport | LatchpmReport | ProjectWorkflowReport, options: GlobalOptions): void {
  if (options.json) {
    process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
    return;
  }
  ...
  if (report.tool === "latchpm") {
    process.stdout.write(`${formatLatchpmReport(report as LatchpmReport)}\n`);
    return;
  }
...
async function askForCommandApproval(title: string, command: NpmCommand): Promise<boolean> {
...
function printCommandBeforeExecution(command: NpmCommand): void {
  process.stderr.write(`latchpm: running ${formatNpmCommand(command)}\n`);
}

async function askForDecision(report: LatchpmReport, policyDecision: PolicyDecision): Promise<Extract<InstallMode, "normal" | "ignore-scripts"> | "denied"> {
  for (;;) {
...
function formatDecisionSummary(report: LatchpmReport, policyDecision: PolicyDecision): string {
...
function formatFindings(report: LatchpmReport): string {
  return [
    "",
    "Findings",
